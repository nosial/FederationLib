<?php
    ...
    class RedisConnection
    {
        ...
        /**
         * Clear all cached search/listing result sets for a given resource prefix.
         *
         * @param string $resourcePrefix The resource prefix (e.g. 'entity:', 'evidence:').
         * @return void
         * @throws CacheOperationException If there is an error during the operation.
         */
        ...
        /**
         * Returns the configured search/listing result set cache TTL.
         *
         * @return int Time-to-live in seconds.
         */
        public static function getResultCacheTtl(): int
        {
            return Configuration::getRedisConfiguration()->getSearchCacheTtl();
        }

        /**
         * Caches an array of serialized record arrays as search/listing results.
         *
         * @param string $cacheKey The cache key generated by getSearchCacheKey().
         * @param array $serializedRecords Array of record arrays (from toArray()).
         * @param int|null $ttl Time-to-live in seconds (null = use config default).
         * @throws CacheOperationException If there is an error during the operation.
         */
        public static function cacheSearchResults(string $cacheKey, array $serializedRecords, ?int $ttl = null): void
        {
            $redis = self::getConnection();
            if ($redis === null)
            {
            ...
            if ($ttl === null)
            {
                $ttl = self::getResultCacheTtl();
            }

            try
            {
                $redis->setex($cacheKey, $ttl, json_encode($serializedRecords, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
            }
            catch (RedisException $e)
            {
                Logger::log()->error(sprintf("Failed to cache search results with key '%s': %s", $cacheKey, $e->getMessage()), $e);
                if(Configuration::getRedisConfiguration()->shouldThrowOnErrors())
                {
                    throw new CacheOperationException(sprintf("Failed to cache search results with key '%s'", $cacheKey), 0, $e);
                }
            }
        }
         ...
         *
         * @param string $cacheKey The cache key generated by getSearchCacheKey().
         * @return array|null The cached array of record arrays, or null if not found.
         * @throws CacheOperationException If there is an error during the operation.
         */
        public static function getCachedSearchResults(string $cacheKey): ?array
        {
            $redis = self::getConnection();
            if ($redis === null)
            {
            ...
            {
                Logger::log()->error(sprintf("Failed to retrieve cached search results with key '%s': %s", $cacheKey, $e->getMessage()), $e);
                if(Configuration::getRedisConfiguration()->shouldThrowOnErrors())
                {
                    throw new CacheOperationException(sprintf("Failed to retrieve cached search results with key '%s'", $cacheKey), 0, $e);
                }
